...
def _get_remote_metrics(host: str) -> dict:
    ...
    try:
        ...
        return {
            ...
            "ram": {"total": round(float(ram_total), 1), "used": round(float(ram_used), 1), "percent": round(float(ram_pct), 1)},
            "disk": {"total": round(float(disk_total), 1), "used": round(float(disk_used), 1), "percent": round(float(disk_pct), 1)},
            "uptime": uptime,
